...
FIGURES = [
    ...
    ("Fig5_clinical_robustness",
     ...
     "detection yield --- of all impending hypotension events, the fraction flagged at least $t$ min "
     "ahead at a fixed operating point (spec.\\ $\\geq$0.90); the $t{=}0$ intercept is sensitivity and "
     "the decay is the lead-time distribution (dotted guides at the clinically-cited 2 and 5 min). "
     "Trained baselines flag more events at every lead time; TiRex-2 is competitive and above the best "
     "zero-shot foil. (d) Operating characteristics at specificity $\\geq$0.90: a 1$\\times$4 strip, one "
     "panel per metric (sensitivity, PPV, NPV, F1), the four models overlaid across horizons. (b) "
     "Severity-stratified detection: a 1$\\times$4 strip of AUROC vs horizon, one panel per hypotension "
     "threshold/duration (MAP$<$65, $<$55, $<$50 mmHg, and $<$65 mmHg sustained $\\geq$5 min). (c) "
     "Subgroup robustness: per-subgroup hypotension AUROC at 5 min --- TiRex-2 (point + case-clustered "
     "95\\% CI) with TFT, PatchTST and Chronos-Bolt overlaid; TiRex-2 stays close to its overall value "
     "across every subgroup. Trained baselines are all-cases 5-fold out-of-fold; TiRex-2 and "
